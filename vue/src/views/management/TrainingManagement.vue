<template>
  <section class="module-page module-composed-page">
    ...
    <el-dialog title="创建实训环境" :visible.sync="createDialog" width="520px"><el-form :model="form" label-width="110px"><el-form-item label="用户编号"><el-input v-model.number="form.userId" type="number" /></el-form-item><el-form-item label="课程编号"><el-input v-model.number="form.courseId" type="number" /></el-form-item><el-form-item label="处理服务器"><el-select v-model="form.agentId" filterable placeholder="选择服务器" style="width:100%"><el-option v-for="agent in agents" :key="agent" :label="agent" :value="agent" /></el-select></el-form-item><el-form-item label="槽位编号"><el-select v-model="form.slotNumber" placeholder="选择可用槽位" style="width:100%"><el-option v-for="slot in availableSlots" :key="slot.agentId + '-' + slot.slotNumber" :label="'槽位 ' + slot.slotNumber + (slot.userId ? '（已绑定用户 ' + slot.userId + '）' : '（可用）')" :value="slot.slotNumber" :disabled="!!slot.userId" /></el-select></el-form-item><el-form-item label="标注模板"><el-input v-model="form.annotationTemplateId" /></el-form-item><el-form-item label="标注版本"><el-input v-model.number="form.annotationTemplateVersion" type="number" /></el-form-item><el-form-item label="编辑模板"><el-input v-model="form.editorTemplateId" /></el-form-item><el-form-item label="编辑版本"><el-input v-model.number="form.editorTemplateVersion" type="number" /></el-form-item></el-form><span slot="footer"><el-button @click="createDialog=false">取消</el-button><el-button type="primary" :loading="creating" @click="create">创建</el-button></span></el-dialog>
  </section>
</template>
...
export default {
  data: () => ({ loading: false, busyId: '', creating: false, createDialog: false, environments: [], slots: [], form: { userId: null, courseId: null, agentId: '', slotNumber: null, annotationTemplateId: '', annotationTemplateVersion: null, editorTemplateId: '', editorTemplateVersion: null } }),
  computed: { agents () { return [...new Set(this.slots.map(slot => slot.agentId))] }, availableSlots () { return this.slots.filter(slot => !this.form.agentId || slot.agentId === this.form.agentId) } },
  created () { this.load(); this.loadSlots() },
  methods: {
    ...
    async loadSlots () { const result = await listAdminTrainingSlots(); this.slots = result.data || [] },
