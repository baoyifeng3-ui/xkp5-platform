  ...
  computed: {
    ...
    participantItems () {
      return [
        ...
        { key: 'practical', activeKey: 'practical', label: '比赛实操', icon: 'el-icon-monitor', destination: 5 }
      ]
    },
    activePaperLabel () {
      return this.$store.state.Match.activePaper ? `${this.$store.state.Match.activePaper} 卷 · 进行中` : '当前赛卷未选择'
    },
    shellItems () {
