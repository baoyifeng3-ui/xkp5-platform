<template>
    <div>
        <el-carousel v-if="!showDetect" trigger="click" height="400px" :autoplay="false">
            <el-carousel-item v-for="item in 10" :key="item">
                <img :src="require(`../assets/data_set_A/${item}.jpg`)" alt="" />
            </el-carousel-item>
        </el-carousel>
        <el-carousel v-else trigger="click" height="400px" :autoplay="false">
            <el-carousel-item v-for="(item, index) in imgSrc" :key="index">
                <img :src="item" alt="" />
            </el-carousel-item>
        </el-carousel>
        <div class="d-testing">
            <el-button type="primary" @click="doCheckUtils" :disabled="previewOnly" v-loading.fullscreen.lock="fullscreenLoading"
...
import { getToken, removePlan, removeUserName } from "@/utils/auth";
...
export default {
    ...
    methods: {
        ...
        async doTesting () {
            ...
            this.resetDetection();
            try {
                const responses = await Promise.all(base64ListA.map((v, i) => {
