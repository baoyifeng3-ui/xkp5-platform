...
export default {
  ...
  methods: {
    ...
    async review (artifact, decision) { let reason = ''; if (decision === 'REJECT') { const result = await this.$prompt('填写驳回原因，归档将保留到清理策略执行。', '驳回镜像', { inputType: 'textarea', inputValidator: value => Boolean(value && value.trim()) || '请填写驳回原因' }); reason = result.value } else await this.$confirm('批准后导入任务会异步写入内部镜像仓库。是否继续？', '批准镜像', { type: 'warning' }); await reviewImageArtifact(artifact.artifactId, decision, reason); await this.loadAll() },
