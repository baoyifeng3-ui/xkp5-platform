...
assert.ok(view.includes('() => this.refreshActiveWork()'), 'active import and deployment polling must retain component context')
assert.ok(view.includes('.catch(() => {})'), 'deployment polling errors must not create unhandled rejections')
assert.ok(view.includes('<el-progress'), 'importing artifacts must render a progress bar')
assert.ok(view.includes('importProgress'), 'import progress returned by the backend must be visible')
assert.ok(view.includes('importCompletedLayers'), 'copied and total layer counts must be visible')
assert.ok(view.includes('importUpdatedAt'), 'stale import detection must use the backend heartbeat')
assert.ok(view.includes('refreshActiveWork'), 'artifact polling must continue while an import is active')
