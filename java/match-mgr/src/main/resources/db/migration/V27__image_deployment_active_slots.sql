...
UPDATE image_deployment SET idempotency_key = SUBSTRING(active_deployment_key,
        CHAR_LENGTH(agent_id) + CHAR_LENGTH(component_type) + 3)
    WHERE idempotency_key IS NULL
      AND active_deployment_key LIKE CONCAT(agent_id, ':', component_type, ':%');
