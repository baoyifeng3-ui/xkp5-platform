...
## 2026-09-22 当前修复与部署状态

- 源码归档：配套 Agent 修复提交 `de770cf2a5027e027f107f689bf18495c327019d`（版本 0.2.35）。本轮仅提交修复，未制作新发布包。
- 平台修复提交 `bee9c3a` 与在线部署手册提交 `8498760` 已推送 `origin/main`；平台远端主分支当前为 `84987605be048641167ae52936c24f3d58250ad3`。Agent 远端主分支为 `de770cf2a5027e027f107f689bf18495c327019d`。5.1.0 标签仍冻结，未创建 5.1.1 或新离线包。
- 在线部署、升级、备份、回滚和验收步骤已整理至 `docs/ONLINE_DEPLOYMENT_AND_UPGRADE_GUIDE.md`，命令使用占位符，不含凭据。

- F01/F04/F06/F07/F08 已部署；Java 当前镜像 `qa-safe-0235-20260922`、Vue `qa-safe-20260921`。Java 750 项、Vue 50 脚本及生产构建已有通过记录；升级版本变更后 clean AgentUpgradeServiceTest 2/2 通过。测试通过不等于全功能页面验收。
- F05 已部署三台 Agent 0.2.35，全部在线。三台通过正常平台 DELETE inventory/images API 删除遗留 QA 镜像（c8023ac7b516 前缀）并回报 SUCCEEDED；SSH 实查完整 ID 与两个标签均消失，刷新后平台清单无 QA 镜像。每台删除前后所有容器 ID/状态相同（24/24/0 个）。重复升级请求均被拒绝。
- F05 采用无 Force 逐标签删除，删除前验证 ID、删除后验证镜像不存在；不清理父镜像。Agent race 全量、vet、静态 Linux amd64 构建及本机真实 Docker 双标签/停止容器保护测试通过。外部 tag/load 并发无法做到原子删除，今后执行仍应在无外部镜像写入窗口；部分失败需刷新清单，不宣称原子回滚。
- 备份保留：本机 `backups/qa-deploy-20260921-224951/match.sql`、旧 Java/Vue 容器；每台远端 `/root/xkp-agent-backup-before-0235-20260922` 和 `/usr/local/bin/xkp-agent.before-0235-20260922`。Agent 0.2.35 SHA-256：c293927c26cc59b97a516b3646f58918be1106869f141d199f64fb7b132fe458。
- F03 三个部署脚本 Git 执行位已修复，证书脚本通过；升级静态测试在临时副本统一 Makefile 换行后通过。用户选择本轮仅整理并提交修复，不生成新发布包；5.1.0 标签、发布清单及原离线包保持冻结版本内容。
- 最新冒烟：学生登录与 43 门课程读取正常，前后端 HTTP 200。未执行全功能浏览器测试、大文件压力、全新机器安装升级回滚。
- F02 内嵌工具、T100 模型问题按用户要求待定，不修改。

以下为此前阶段记录，冲突时以上述当前状态为准。

## 2026-09-20 续测状态

- 2026-09-21 继续修复 QA 缺陷：F05 Agent 镜像删除先检查所有容器引用，再逐个移除该镜像 ID 的全部标签，不使用 Force，保留 Docker 引用保护；Agent 修改未部署。Java 全量回归退出 0，Vue 契约脚本 50/50 通过，Agent 定向回归通过。Agent 全量回归仍需独立依赖缓存或网络。
- 本轮仍未擅自修改 F02 内嵌工具或 T100 模型推理：分别缺少可用浏览器认证和已部署模型证据。F04 负等待已用前端非负显示保护修复并通过契约测试。F01/F03/F05/F06/F07/F08 的本地修复仍未部署，不能写成运行环境已解决。

- 本地修复 F01 JSON 请求、F06 当前转换接口错误信息、F07 账号删除清理进度、F08 草稿章节校验；Java 154 类 750 项全部通过，新增前端 JSON 行为回归通过。尚未部署，旧运行版本缺陷未关闭。
- 2026-09-21 回归：Python 5/5、Go dockerinventory/protocol、Vue 修复行为通过；Java 154 类 750 项全量通过。修复仍未部署，F05 多标签镜像删除未修复。
- Agent 按标签删除修改因安全风险撤回，仓库无内容差异；F05 及远端测试镜像残留仍待解决。授权下载 400/409 差异为字节数误读，已更正。

- 最新补测：跨用户启动实训及不存在资源拒绝通过；发现 F08 草稿课程章节可被学生按 ID 直接读取，测试草稿/账号已清理。授权请求创建、下载和非法 ID 拒绝通过；重复下载 HTTP 400 符合一次性语义，407/409 为内容字节数，撤回状态差异误报。

- 测试重新进行中。模板版本操作、四类题目管理、章节管理已补测通过；浏览器仍认证阻塞。详见统一报告。
- 误写报告已从 MySQL 完整 binlog 前镜像恢复，CRC 校验与当前值比对后通过 API 恢复，回读原文字节一致；临时恢复文件已删除。
- 学习进度 30%/100% 提交与回读通过；发现删除账号遗留 course_resource_progress，测试行已精确清理。镜像残留与其他验收缺口仍未解决。


## 2026-09-16 测试记录

- 本轮测试结案，详见 `docs/QA_TEST_SUMMARY_2026-09-16.md`；不代表全功能验收通过，未覆盖项保留。
- Java 747 项、Vue 49 脚本、Python 5 项、数据库 9 项、Agent 14 个含测试包通过；585 个 PDF 资源可访问。课堂/比赛往返、课程资源与账户生命周期、作业隔离、工具协议流程已有实测。
- 主要未修复问题：模型 URL 提交 415、多标签镜像删除失败、部分部署脚本执行权限、错误响应堆栈；内嵌异常待浏览器工具恢复后复测。
- 结案核验：TRAINING，课堂关闭，24 环境 STOPPED，临时 QA 账号/课程为零；三台各一个双标签 QA 镜像因删除缺陷残留。
- 未修改应用源码；完整逐按钮/比赛答题评分、有效授权到期、大文件压力、模型推理、全新安装升级回滚未全部验收。
